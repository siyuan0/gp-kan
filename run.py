from lib import model as gplib
from lib import runner
from lib.dataset import MNIST

model = gplib.GP_Model(
    [
        gplib.GP_conv2D(
            28, 28, 1, 1, kernel_size=4, stride=2, num_gp_pts=10, use_double_layer=True
        ),
        gplib.NormaliseGaussian(),
        gplib.GP_conv2D(
            13, 13, 1, 1, kernel_size=3, stride=2, num_gp_pts=10, use_double_layer=True
        ),
        gplib.NormaliseGaussian(),
        gplib.ReshapeGaussian([-1, 36]),
        gplib.LayerFused(36, 10, num_gp_pts=10),
        gplib.NormaliseGaussian(),
        gplib.LayerFused(10, 10, num_gp_pts=10),
    ],
    saveload_path="model.pt",
)

train_dataloader, val_dataloader, test_dataloader = MNIST.get_train_val_test(
    batch_size=64, train=50000, val=10000, test=10000
)
model.print_model()
runner.train_model(
    train_dataloader,
    val_dataloader,
    model,
    epochs=30,
    learning_rate=1e-2,
    use_gpu=True,
    pretrain=True,
    global_jitter=1e-2,
    baseline_jitter=1e-2,
)
model.save_model()
# runner.test_model(test_dataloader, model, use_gpu=True)
